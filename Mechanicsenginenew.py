...
# convert Ui file made in Qt designer to Python script ↓
# python pyside6-uic [FILE NAME].ui -o [FILE NAME]_ui.py

# Import UI made in QT designer
from MechEnginehome.MechanicsEngine_Mainscreen_ui import Ui_MEMainWindow
from Unitconverter.UnitConverter_ui import Ui_UnitConverter
from Momentsofinertia.MOIshapeselector_ui import Ui_MOIshapeselect
from Momentsofinertia.MOIBasiccomp_ui import Ui_MOIBasiccomp
from Momentsofinertia.MOIadvcompsteel_ui import Ui_MOIadvcompsteel
from Momentsofinertia.MOIadvcompalwd_ui import Ui_MOIadvcompalwd
from Momentsofinertia.MOIBasicSQcomp_ui import Ui_MOIBasicSQcomp
from Determinatebeams.Shearandmomentdiagramming_ui import Ui_shearandmomentscalculator
from MechEnginehome.credits_ui import Ui_credits

class MechEngHome(QMainWindow, Ui_MEMainWindow, QPushButton):        
    def __init__(self):
        super().__init__()
        ...
        #set MechEngHome Window title
        self.setWindowTitle('Mechanics Engine (alpha v0.3.6)')
        ...
        self.Modulelist.itemClicked.connect(self.on_tree_item_clicked)
        
        self.Modulelistdock.setWindowTitle('Module List')
        
        self.window_mapping = {
            # Documents category
            "Unit Converter": self.open_UnitConverter,
            "Determinate Beams": self.open_DeterminateBeams,
    ...
    def on_tree_item_clicked(self, item, column):
        ...
        # Skip if it's a top-level item (category) or not in our mapping
        if item.parent() is None or item_text not in self.window_mapping:
            return
        
        # Get the full path of the item (including parent) for window tracking
        item_path = self.get_item_path(item)
        
        # Check if window already exists
            # Create a new window using the mapping function
        window_creator = self.window_mapping[item_text]
        sub_window = window_creator(item_path)
    ...
    def open_UnitConverter(self, title):
        """Create a new MDI sub window with content"""
        # Create a sub window
        sub_window = UnitConverter()
        sub_window.setWindowTitle("Unit Converter")
        
        sub_window.setAttribute(Qt.WA_DeleteOnClose)  # Ensure proper cleanup on close
        
        sub_window.setFixedSize(690, 335)
    ...
    def open_DeterminateBeams(self, title):
        """Create a new MDI sub window with content"""
        # Create a sub window
        sub_window = Determinatebeams()
        sub_window.setWindowTitle("Determinate Beams Module")
        sub_window.setAttribute(Qt.WA_DeleteOnClose)  # Ensure proper cleanup on close
    ...
    def credits(self):
        self.credits_scr = credits()
        self.credits_scr.show()
    
    def closeEvent(self, event):
        # Ask for confirmation before closing
        confirmation = QMessageBox.question(self, "Confirmation", "Do You Want To Quit?", QMessageBox.Yes | QMessageBox.No)
...
class UnitConverter(QMainWindow, Ui_UnitConverter, QPushButton, QAction):
    inter = 0
    NUMunit_output = 0
    def __init__(self):
        super().__init__()
        
        self.MechEngHome = MechEngHome()
        
        self.ui = Ui_UnitConverter()
        self.setupUi(self)
        
        self.setWindowTitle('Mechanics Engine (alpha v 1.0.0) Unit Converter')
    ...
    # inter - short for intermediate. calculation is stored as an intermediate number then converted to the units specified
        ...
        if units_from == "Newton/Square Centimeter [N/cm\u00b2]":
            inter = NUMinput * 10000
        if units_from == "Millibar [mbar]":
            inter = NUMinput * 100
        if units_from == "Microbar [µbar]" or units_from == "Dyne/Square Centimeter [dyn/cm\u00b2]":
            inter = NUMinput * 0.1
    ...
    #converting from standard to output unit
        ...
        if units_to == "Newton/Square Centimeter [N/cm\u00b2]":
            NUMout = inter / 10000
        if units_to == "Millibar [mbar]":
            NUMout = inter / 100
        if units_to == "Microbar [µbar]" or units_to == "Dyne/Square Centimeter [dyn/cm\u00b2]":
            NUMout = inter / 0.1
...
class MOImodule(QMainWindow, Ui_MOIshapeselect, QAction):
    def __init__(self):
        super().__init__()
        
        self.MechEngHome = MechEngHome()
        
        self.ui = Ui_MOIshapeselect()
        self.setupUi(self)
        
        self.setWindowTitle('Mechanics Engine (alpha v0.0.1) Moment of Inertia Module')
        
        self.actionUnit_Converter.triggered.connect(self.UnitConverter)
        
        self.actionQuit.triggered.connect(self.closeEvent)
        self.actionQuit_All.triggered.connect (self.closeEvent_all)
        
        self.squarerectangle.clicked.connect(self.MOIBasicSQ)
        self.Circle.clicked.connect(self.MOIBasicCirc)
        self.box.clicked.connect(self.MOIBasicBox)
        self.Tubepipe.clicked.connect(self.MOIBasicPipe)
        self.Customflanged.clicked.connect(self.MOIBasicFlanged)
        self.Customchannel.clicked.connect(self.MOIBasicChannel)
        self.Ishape.clicked.connect(self.MOIBasicI)
        self.Lshape.clicked.connect(self.MOIBasicL)
        self.Cshape.clicked.connect(self.MOIBasicC)
        self.Tshape.clicked.connect(self.MOIBasicT)
        self.stiffTshape.clicked.connect(self.MOIBasicStiffT)
        self.StiffTshapeup.clicked.connect(self.MOIBasicStiffTup)
        self.Zshape.clicked.connect(self.MOIBasicZ)
        self.reversedZ.clicked.connect(self.MOIBasicrevZ)
        self.Flaredchannel.clicked.connect(self.MOIBasicFlared)
        self.DoubleI.clicked.connect(self.MOIBasicDI)
        self.DoubleC.clicked.connect(self.MOIBasicDC)
        self.DoubleCtoetoe.clicked.connect(self.MOIBasicDCtoetoe)
        self.DoubleT.clicked.connect(self.MOIBasicDT)
        self.DoubleL.clicked.connect(self.MOIBasicDL)
        
        self.usFlanged.clicked.connect(self.MOIadvUSFlanged)
        self.usChannel.clicked.connect(self.MOIadvUSChannel)
        self.usT.clicked.connect(self.MOIadvUSTee)
        self.usAngle.clicked.connect(self.MOIadvUSAngle)
        self.usDoubleangle.clicked.connect(self.MOIadvUSDoubleAngle)
        self.usRecthollowsection.clicked.connect(self.MOIadvUSRectHSec)
        self.usPipe.clicked.connect(self.MOIadvUSPipe)
    
        self.metricFlanged.clicked.connect(self.MOIadvMetricFlanged)
        self.metricChannel.clicked.connect(self.MOIadvMetricChannel)
        self.metricT.clicked.connect(self.MOIadvMetricT)
        self.metricAngle.clicked.connect(self.MOIadvMetricAngle)
        self.metricDoubleAngle.clicked.connect(self.MOIadvMetricDoubleAngle)
        self.metricRectholloesection.clicked.connect(self.MOIadvMetricRectHSec)
        self.metricPipe.clicked.connect(self.MOIadvMetricPipe)
        
        self.euroFlanged.clicked.connect(self.MOIadvEuroFlanged)
        self.euroChannel.clicked.connect(self.MOIadvEuroChannel)
        self.euroRecthollowsection.clicked.connect(self.MOIadvEuroRectHSec)
        
    def MOIBasicSQ(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicSQ()
        self.MOIBasiccomp.show()
    
    def MOIBasicCirc(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicCirc()
        self.MOIBasiccomp.show()
    
    def MOIBasicBox(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicBox()
        self.MOIBasiccomp.show()
    
    def MOIBasicPipe(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicPipe()
        self.MOIBasiccomp.show()
    
    def MOIBasicFlanged(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicFlanged()
        self.MOIBasiccomp.show()
    
    def MOIBasicChannel(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicChannel()
        self.MOIBasiccomp.show()
    
    def MOIBasicI(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicI()
        self.MOIBasiccomp.show()
    
    def MOIBasicL(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicL()
        self.MOIBasiccomp.show()
    
    def MOIBasicC(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicC()
        self.MOIBasiccomp.show()
    
    def MOIBasicT(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicT()
        self.MOIBasiccomp.show()
    
    def MOIBasicStiffT(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicStiffT()
        self.MOIBasiccomp.show()
    
    def MOIBasicStiffTup(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicStiffTup()
        self.MOIBasiccomp.show()
    
    def MOIBasicZ(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicZ()
        self.MOIBasiccomp.show()
    
    def MOIBasicrevZ(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicrevZ()
        self.MOIBasiccomp.show()
    
    def MOIBasicFlared(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicFlared()
        self.MOIBasiccomp.show()
    
    def MOIBasicDI(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicDI()
        self.MOIBasiccomp.show()
    
    def MOIBasicDC(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicDC()
        self.MOIBasiccomp.show()
    
    def MOIBasicDCtoetoe(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicDCtoetoe()
        self.MOIBasiccomp.show()
    
    def MOIBasicDT(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicDT()
        self.MOIBasiccomp.show()
    
    def MOIBasicDL(self):
        self.hide()
        self.MOIBasiccomp = MOIBasicDL()
        self.MOIBasiccomp.show()
    
    def MOIadvUSFlanged(self):
        self.hide()
        self.MOIadvcomp = MOIadvUSFlanged()
        self.MOIadvcomp.show()
    
    def MOIadvUSChannel(self):
        self.hide()
        self.MOIadvcomp = MOIadvUSChannel()
        self.MOIadvcomp.show()
    
    def MOIadvUSTee(self):
        self.hide()
        self.MOIadvcomp = MOIadvUSTee()
        self.MOIadvcomp.show()
    
    def MOIadvUSAngle(self):
        self.hide()
        self.MOIadvcomp = MOIadvUSAngle()
        self.MOIadvcomp.show()
    
    def MOIadvUSDoubleAngle(self):
        self.hide()
        self.MOIadvcomp = MOIadvUSDoubleAngle()
        self.MOIadvcomp.show()
    
    def MOIadvUSRectHSec(self):
        self.hide()
        self.MOIadvcomp = MOIadvUSRectHSec()
        self.MOIadvcomp.show()
    
    def MOIadvUSPipe(self):
        self.hide()
        self.MOIadvcomp = MOIadvUSPipe()
        self.MOIadvcomp.show()
    
    def MOIadvMetricFlanged(self):
        self.hide()
        self.MOIadvcomp = MOIadvMetricFlanged()
        self.MOIadvcomp.show()
    
    def MOIadvMetricChannel(self):
        self.hide()
        self.MOIadvcomp = MOIadvMetricChannel()
        self.MOIadvcomp.show()
    
    def MOIadvMetricT(self):
        self.hide()
        self.MOIadvcomp = MOIadvMetricT()
        self.MOIadvcomp.show()
    
    def MOIadvMetricAngle(self):
        self.hide()
        self.MOIadvcomp = MOIadvMetricAngle()
        self.MOIadvcomp.show()
    
    def MOIadvMetricDoubleAngle(self):
        self.hide()
        self.MOIadvcomp = MOIadvMetricDoubleAngle()
        self.MOIadvcomp.show()
    
    def MOIadvMetricRectHSec(self):
        self.hide()
        self.MOIadvcomp = MOIadvMetricRectHSec()
        self.MOIadvcomp.show()
    
    def MOIadvMetricPipe(self):
        self.hide()
        self.MOIadvcomp = MOIadvMetricPipe()
        self.MOIadvcomp.show()
    
    def MOIadvEuroFlanged(self):
        self.hide()
        self.MOIadvcomp = MOIadvEuroFlanged()
        self.MOIadvcomp.show()
    
    def MOIadvEuroChannel(self):
        self.hide()
        self.MOIadvcomp = MOIadvEuroChannel()
        self.MOIadvcomp.show()
    
    def MOIadvEuroRectHSec(self):
        self.hide()
        self.MOIadvcomp = MOIadvEuroRectHSec()
        self.MOIadvcomp.show()
    ...
    def closeEvent(self, event):
        self.close()
        self.MechEngHome.show()
    
    def closeEvent_all(self, event):
        self.close()
        self.MechEngHome.close()
    
class MOIBasicSQ(QMainWindow, Ui_MOIBasicSQcomp, QAction):
    def __init__(self):
        super().__init__()
        
        self.MechEngHome = MechEngHome()
        self.MOIshapeselect = MOImodule()
        
        self.ui = Ui_MOIBasicSQcomp()
        self.setupUi(self)
        ...
        self.actionBack_to_Shape_Selector.triggered.connect(self.MOIselect)
        self.actionQuit.triggered.connect(self.closeEvent)
        self.actionQuit_All.triggered.connect (self.closeEvent_all)
        
        self.yzaxis.setChecked(True)
        self.rotate0deg.setChecked(True)
    
        self.scene = QGraphicsScene(self)
        self.Shapedisplay.setScene(self.scene)
        
        placeholder = QGraphicsRectItem(0, 0, 200, 400)
        placeholder.setBrush(QtGui.QBrush(QtGui.QColor(100, 100, 250)))
        ...
        self.scene = QGraphicsScene(self)
        self.Shapedisplay.setScene(self.scene)
        self.computeMOI.clicked.connect(self.draw_rectangle)
        self.computeMOI.clicked.connect(self.calculate_MOI)
        self.rectheight.valueChanged.connect(self.calculate_MOI)
        self.rectwidth.valueChanged.connect(self.calculate_MOI)
        self.yzaxis.toggled.connect(self.preMOIcalcs)
        self.xyaxis.toggled.connect(self.preMOIcalcs)
        
        self.Unitscombobox.clear()
        self.Unitscombobox.addItems(["in.", "mm"])
        self.Unitscombobox.currentTextChanged.connect(self.calculate_MOI)
        
        self.matlelasticmod.clear()
        self.matlelasticmod.addItems(["", "materials"])
        self.Unitscombobox.currentTextChanged.connect(self.calculate_MOI)

    def draw_rectangle(self):
        # Get width and height from double spin boxes
        width = self.rectwidth.value()
        height = self.rectheight.value()
        ...
        self.Shapedisplay.setSceneRect(0, 0, width, height)
        self.Shapedisplay.fitInView(rect_item, QtCore.Qt.KeepAspectRatio)
    
    def preMOIcalcs(self):
        self.axisprops.clear()
        if self.yzaxis.isChecked():
            self.axisprops.addItems(["Z Axis Properties", "Y Axis Properties"])
        if self.xyaxis.isChecked():
            self.axisprops.addItems(["X Axis Properties", "Y Axis Properties"])
        
        self.axisprops.currentTextChanged.connect(self.calculate_MOI)
    
    def calculate_MOI(self):
        height = self.rectheight.value()
        width = self.rectwidth.value()
        axisprop = self.axisprops.currentText()
        material = self.matlelasticmod.currentText()
        units = self.Unitscombobox.currentText()
        
        if axisprop == "Z Axis Properties" or "X Axis Properties":
            bcent = height / 2
        self.bottomtocentroid.setValue(bcent)
        
        if axisprop == "Z Axis Properties" or "X Axis Properties":
            tcent = height / 2
        self.toptocentroid.setValue(tcent)
        
        if axisprop == "Y Axis Properties":
            bcent = width / 2
        self.bottomtocentroid.setValue(bcent)
        ...
        if axisprop == "X Axis Properties":
            self.MOIlabel.setText("Ix")
        
        if axisprop == "Y Axis Properties":
            self.MOIlabel.setText("Iy")
        
        if axisprop == "Z Axis Properties":
            self.MOIlabel.setText("Iz")
        
        if axisprop == "X Axis Properties" and self.xyaxis.isChecked():
            self.bottomtocentroidvar.setText("y(bot)")
            self.toptocentroidvar.setText("y(top)")
            self.bottomtocentroidlabel.setText("Distance From Bottom to Centroid")
            self.toptocentroidlabel.setText("Distance From Centroid to Top")
            
        if axisprop == "Y Axis Properties" and self.xyaxis.isChecked():
            self.bottomtocentroidvar.setText("x(left)")
            self.toptocentroidvar.setText("x(right)")
            self.bottomtocentroidlabel.setText("Distance From Left to Centroid")
            self.toptocentroidlabel.setText("Distance From Centroid to Right")
        
        if axisprop == "Y Axis Properties" and self.yzaxis.isChecked():
            self.bottomtocentroidvar.setText("z(left)")
            self.toptocentroidvar.setText("z(right)")
            self.bottomtocentroidlabel.setText("Distance From Left to Centroid")
            self.toptocentroidlabel.setText("Distance From Centroid to Right")
        
        if axisprop == "Z Axis Properties" and self.yzaxis.isChecked():
            self.bottomtocentroidvar.setText("y(bot)")
            self.toptocentroidvar.setText("y(top)")
            self.bottomtocentroidlabel.setText("Distance From Bottom to Centroid")
            self.toptocentroidlabel.setText("Distance From Centroid to Top")
        ...
        if units == "in.":
            self.bottomtocentroidunits.setText("in.")
            self.toptocentroidunits.setText("in.")
            self.areaunits.setText("in.\u00b2")
            self.MOIunits.setText("in.\u2074")
            self.sectionmodunits.setText("in.\u00b3")
            self.sectionmodbottomunits.setText("in.\u00b3")
            self.sectionmodtopunits.setText("in.\u00b3")
            self.radofgyrationunits.setText("in.")
            self.plasticmodunits.setText("in.\u00b3")
            self.plastictobottomunits.setText("in.")
            self.plastictotopunits.setText("in.")
            self.polarMOIunits.setText("in.\u2074")
            self.productofinertiaunits.setText("in.\u2074")
            self.maxMOIunits.setText("in.\u2074")
            self.minMOIunits.setText("in.\u2074")
            self.axisangleunits.setText("degrees")
        
        if units == "mm":
            self.bottomtocentroidunits.setText("mm")
            self.toptocentroidunits.setText("mm")
            self.areaunits.setText("mm\u00b2")
            self.MOIunits.setText("mm\u2074")
            self.sectionmodunits.setText("mm\u00b3")
            self.sectionmodbottomunits.setText("mm\u00b3")
            self.sectionmodtopunits.setText("mm\u00b3")
            self.radofgyrationunits.setText("mm")
            self.plasticmodunits.setText("mm\u00b3")
            self.plastictobottomunits.setText("mm")
            self.plastictotopunits.setText("mm")
            self.polarMOIunits.setText("mm\u2074")
            self.productofinertiaunits.setText("mm\u2074")
            self.maxMOIunits.setText("mm\u2074")
            self.minMOIunits.setText("mm\u2074")
            self.axisangleunits.setText("degrees")
        
        if material == "":
            self.elasticmod.setValue(1)
        
        self.area.setValue(width*height)
            
        self.MOI.setValue(((width)*(height**3))/12)
        
        self.radofgyration.setValue(height/(math.sqrt(12)))
        
        self.polarMOI.setValue((((width)*(height**3))/12)+(((height)*(width**3))/12))
        
        self.sectionmod.setValue((width*(height**2))/6)
        
        self.plasticmod.setValue((width*(height**2))/4)
        
        self.shapefactor.setValue((((width*2) + (height*2))**2)/(width * height))
...
class MOIBasicCirc(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicBox(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicPipe(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicFlanged(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicChannel(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicI(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicL(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicC(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicT(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicStiffT(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicStiffTup(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicZ(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicrevZ(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicFlared(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicDI(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicDC(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicDCtoetoe(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicDT(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIBasicDL(QMainWindow, Ui_MOIBasiccomp, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvUSFlanged(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvUSChannel(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvUSTee(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvUSAngle(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvUSDoubleAngle(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvUSRectHSec(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvUSPipe(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvMetricFlanged(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvMetricChannel(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvMetricT(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvMetricAngle(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvMetricDoubleAngle(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvMetricRectHSec(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvMetricPipe(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvEuroFlanged(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvEuroChannel(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvEuroRectHSec(QMainWindow, Ui_MOIadvcompsteel, QAction):
    def __init__(self):
        super().__init__()
...
class MOIadvalwd(QMainWindow, Ui_MOIadvcompalwd, QAction):
    def __init__(self):
        super().__init__()
        ...
        self.ui = Ui_MOIadvcompalwd
        self.setupUi(self)
        
        self.setWindowTitle('Mechanics Engine (alpha v0.0.1) Moment of Inertia Module')
        
        self.actionUnit_Converter.triggered.connect(self.Unit_converter)
...
class Determinatebeams(QMainWindow, Ui_shearandmomentscalculator, QAction):
    def __init__(self):
        super().__init__()
        ...
        self.setWindowTitle('Mechanics Engine (alpha v0.0.1) Determinate Beams Module')
        ...
        self.scene = QGraphicsScene(self)
        self.Loadsdiagram.setScene(self.scene)
    ...
    def add_concenuploads(self):
        ...
        elementbox.setGeometry(QRect(670, 900, 281, 174))
        sizePolicy1 = QSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        sizePolicy1.setHorizontalStretch(0)
        sizePolicy1.setVerticalStretch(0)
        sizePolicy1.setHeightForWidth(elementbox.sizePolicy().hasHeightForWidth())
        elementbox.setSizePolicy(sizePolicy1)
        ...
        self.concenloadlocation.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.concenloadlocation.setMaximum(100000000.000000000000000)
        self.concenloadlocation.wheelEvent = lambda event: None
        self.verticalLayout_2.addWidget(self.concenloadlocation)
        
        self.concenloadlocationunits = QComboBox(elementbox)
        self.concenloadlocationunits.wheelEvent = lambda event: None
        ...
        self.concenloadmagnitude.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.concenloadmagnitude.setMaximum(100000000.000000000000000)
        self.concenloadmagnitude.wheelEvent = lambda event: None
        self.verticalLayout_3.addWidget(self.concenloadmagnitude)

        self.concenloadmagnitudeunits = QComboBox(elementbox)
        self.concenloadmagnitudeunits.wheelEvent = lambda event: None
        ...
        self.concenloadlocationunits.addItems(["", "in.", "ft", "mm", "m"])
        self.concenloadmagnitudeunits.addItems(["", "lb", "kips", "N", "kN"])
        
        self.scroll_layout.addWidget(elementbox)
        
        elementbox.toggled.connect(lambda checked: self.remove_groupbox(elementbox, checked))
        
    def add_concendownloads(self, font):
    ...
    def add_uniformuploads(self):
        ...
        elementbox.setGeometry(QRect(20, 200, 281, 224))
        sizePolicy1 = QSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        sizePolicy1.setHorizontalStretch(0)
        sizePolicy1.setVerticalStretch(0)
        sizePolicy1.setHeightForWidth(elementbox.sizePolicy().hasHeightForWidth())
        ...
        self.uniformloadstart.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.uniformloadstart.setMaximum(100000000.000000000000000)
        self.uniformloadstart.wheelEvent = lambda event: None
        self.verticalLayout_11.addWidget(self.uniformloadstart)

        self.verticalSpacer = QSpacerItem(30, 31, QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Preferred)
        ...
        self.uniformloadend.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.uniformloadend.setMaximum(100000000.000000000000000)
        self.uniformloadend.wheelEvent = lambda event: None
        self.verticalLayout_12.addWidget(self.uniformloadend)
        self.verticalLayout_13.addLayout(self.verticalLayout_12)

        self.uniformloadunits = QComboBox(elementbox)
        self.uniformloadunits.wheelEvent = lambda event: None
        self.verticalLayout_13.addWidget(self.uniformloadunits)
        ...
        self.uniformloadmagnitude.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.uniformloadmagnitude.setMaximum(100000000.000000000000000)
        self.uniformloadmagnitude.wheelEvent = lambda event: None
        self.verticalLayout_14.addWidget(self.uniformloadmagnitude)

        self.uniformloadmagnitudunits = QComboBox(elementbox)
        self.uniformloadmagnitudunits.wheelEvent = lambda event: None
        ...
        self.uniformloadunits.addItems(["", "in.", "ft", "mm", "m"])
        self.uniformloadmagnitudunits.addItems(["", "lb", "kips", "N", "kN"])
    ...
    def add_lineardistribuploads(self):
        ...
        elementbox.setGeometry(QRect(330, 20, 281, 255))
        sizePolicy2 = QSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        sizePolicy2.setHorizontalStretch(0)
        sizePolicy2.setVerticalStretch(0)
        sizePolicy2.setHeightForWidth(elementbox.sizePolicy().hasHeightForWidth())
        elementbox.setSizePolicy(sizePolicy2)
        elementbox.setFont(font)
        elementbox.setCheckable(True)
        ...
        sizePolicy3.setHeightForWidth(self.lineardistribloadstart.sizePolicy().hasHeightForWidth())
        self.lineardistribloadstart.setSizePolicy(sizePolicy3)
        self.lineardistribloadstart.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.lineardistribloadstart.setMaximum(100000000.000000000000000)
        self.lineardistribloadstart.wheelEvent = lambda event: None
        self.verticalLayout_16.addWidget(self.lineardistribloadstart)

        self.verticalSpacer_2 = QSpacerItem(20, 31, QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Preferred)
        ...
        self.lineardistribloadend.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.lineardistribloadend.setMaximum(100000000.000000000000000)
        self.lineardistribloadend.wheelEvent = lambda event: None
        self.verticalLayout_18.addWidget(self.lineardistribloadend)
        self.verticalLayout_17.addLayout(self.verticalLayout_18)

        self.lineardistribloadunits = QComboBox(elementbox)
        self.verticalLayout_17.addWidget(self.lineardistribloadunits)
        self.lineardistribloadunits.wheelEvent = lambda event: None
        ...
        self.lineardistribloadmagnitudestart.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.lineardistribloadmagnitudestart.setMaximum(100000000.000000000000000)
        self.lineardistribloadmagnitudestart.wheelEvent = lambda event: None
        ...
        self.lineardistribloadmagnitudunits.wheelEvent = lambda event: None
        ...
        self.lineardistribloadmagnitudeend.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.lineardistribloadmagnitudeend.setMaximum(100000000.000000000000000)
        self.lineardistribloadmagnitudeend.wheelEvent = lambda event: None
        ...
        self.lineardistribloadunits.addItems(["", "in.", "ft", "mm", "m"])
        self.lineardistribloadmagnitudunits.addItems(["", "lb", "kips", "N", "kN"])
    ...
    def add_lineardistribdownloads(self):
        ...
        self.lineardistribloadend = QDoubleSpinBox(elementbox)
        self.lineardistribloadend.setMinimumSize(QSize(119, 0))
        self.lineardistribloadend.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.lineardistribloadend.setMaximum(100000000.000000000000000)
        self.lineardistribloadend.wheelEvent = lambda event: None
        self.verticalLayout_18.addWidget(self.lineardistribloadend)
    ...
    def add_momentccw(self):
        ...
        elementbox.setGeometry(QRect(340, 290, 282, 174))
        sizePolicy2 = QSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        sizePolicy2.setHorizontalStretch(0)
        sizePolicy2.setVerticalStretch(0)
        sizePolicy2.setHeightForWidth(elementbox.sizePolicy().hasHeightForWidth())
        ...
        self.label_19 = QLabel("Moment Location\n(x-coordinate)")
        self.label_19.setAlignment(Qt.AlignCenter)
        self.horizontalLayout_12.addWidget(self.label_19)

        self.verticalLayout_21 = QVBoxLayout()
        self.momentlocation = QDoubleSpinBox(elementbox)
        self.momentlocation.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.momentlocation.setMaximum(100000000.000000000000000)
        self.momentlocation.wheelEvent = lambda event: None
        ...
        self.momentlocationunits.wheelEvent = lambda event: None
        ...
        self.label_20.setMinimumSize(QSize(121, 0))
        self.label_20.setAlignment(Qt.AlignCenter)
        self.horizontalLayout_13.addWidget(self.label_20)

        self.verticalLayout_22 = QVBoxLayout()
        self.momentmagnitude = QDoubleSpinBox(elementbox)
        self.momentmagnitude.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.momentmagnitude.setMaximum(100000000.000000000000000)
        self.momentmagnitude.wheelEvent = lambda event: None
        ...
        self.momentmagnitudeunits.wheelEvent = lambda event: None
        ...
        self.momentlocationunits.addItems(["", "in.", "ft", "mm", "m"])
        self.momentmagnitudeunits.addItems(["", "lb-in.", "lb-ft", "kip-in.", "kip-ft", "N-mm", "N-m", "kN-mm", "kN-m"])
    ...
    def varsupportsbeam(self):
        ...
        rect_item = QGraphicsRectItem(0, 0, 550, 50)
        rect_item.setPen(pen)
        rect_item.setBrush(QtGui.QBrush(QtGui.QColor(100, 100, 100)))  # Set color
    ...
    def leftsidebeam(self):
        ...
        line = self.scene.addLine(-3, -50, -3, 100, pen2)
        s1 = self.scene.addLine(-4, -35, -15, -27, pen2)
        s3 = self.scene.addLine(-4, -5, -15, 3, pen2)
        s5 = self.scene.addLine(-4, 25, -15, 33, pen2)
        s7 = self.scene.addLine(-4, 55, -15, 63, pen2)
        s7 = self.scene.addLine(-4, 85, -15, 93, pen2)
    ...
    def rightsidebeam(self):
        ...
        line = self.scene.addLine(553, -50, 553, 100, pen2)
        s1 = self.scene.addLine(554, -35, 565, -27, pen2)
        s3 = self.scene.addLine(554, -5, 565, 3, pen2)
        s5 = self.scene.addLine(554, 25, 565, 33, pen2)
        s7 = self.scene.addLine(554, 55, 565, 63, pen2)
        s7 = self.scene.addLine(554, 85, 565, 93, pen2)
